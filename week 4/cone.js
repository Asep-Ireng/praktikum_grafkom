function generateEllipticCone(a, b, c, tStep, vStep, tMax) {
    var vertices = [];
    var faces = [];

    // Kita buat t dari 0..tMax (positif) dan gambarkan kedua nappes (atas & bawah)
    for (var i = 0; i <= tStep; i++) {
        var t = (i / tStep) * tMax; // 0 .. tMax

        for (var j = 0; j <= vStep; j++) {
            var v = j / vStep * 2 * Math.PI;

            // nappe atas (y positif)
            var x1 = a * t * Math.cos(v);
            var y1 = b * t;
            var z1 = c * t * Math.sin(v);

            vertices.push(x1, y1, z1);
            vertices.push(...[x1, y1, z1].map(val => val / (tMax * Math.max(a,b,c) * 2) + 0.5)); // warna

            // nappe bawah (y negatif)
            var x2 = a * t * Math.cos(v);
            var y2 = -b * t;
            var z2 = c * t * Math.sin(v);

            vertices.push(x2, y2, z2);
            vertices.push(...[x2, y2, z2].map(val => val / (tMax * Math.max(a,b,c) * 2) + 0.5));
        }
    }

    // Faces: sambungkan ring i ke i+1, untuk kedua nappes
    var rowLength = (vStep + 1) * 2; // tiap t punya (vStep+1) * 2 vertex (atas+bawah)
    for (var i = 0; i < tStep; i++) {
        for (var j = 0; j < vStep; j++) {
            ...
            // segitiga untuk nappe atas (menggunakan indeks even: first,second,...)
            faces.push(first, second, fourth);
            faces.push(first, fourth, third);

            // segitiga untuk nappe bawah (indeks odd: first+1, second+1, ...)
            var f2 = first + 1;
            var s2 = second + 1;
            var t2 = third + 1;
            var fo2 = fourth + 1;
...
function main() {
    ...
    /*======================== THE TRIANGLE ======================== */
    // POINTS:
    var cone = generateEllipticCone(0.2, 0.5, 0.2, 60, 60, 2.0);
    var object_vertex = cone.vertices;
    var object_faces = cone.faces;
