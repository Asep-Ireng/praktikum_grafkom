export class Lathe {
  ...
  constructor(GL, LIBS, SHADER_PROGRAM, locations, opts={}){
    ...
    const profileSegments = opts.profileSegments ?? 32;
    this.color = opts.color ?? [0.5,0.5,0.5,1];
    this.shininess = opts.shininess ?? 10.0;
    this.scaleX = opts.scaleX ?? 1.0; this.scaleZ = opts.scaleZ ?? 1.0;
  ...
  _buildGeometry(controlPoints, segments, profileSegments){
    ...
    for(let i=0;i<profilePoints.length;i++){
      ...
      for(let j=0;j<=segments;j++){
        ...
        const pu = [-r0*this.scaleX*su, 0, r0*this.scaleZ*cu];      // ∂p/∂u
        const pv = [ dr*this.scaleX*cu, dy, dr*this.scaleZ*su ];    // ∂p/∂v
        // n = p_u × p_v (balik ke p_v × p_u jika shading terbalik)
  ...
  render(PARENT_MATRIX, PARENT_NORMAL_MATRIX){
    ...
    this.childs.forEach(c=>c.render(MODEL_MATRIX, NORMAL_MATRIX));
  }
}
